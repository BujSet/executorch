...
with open("results.txt", "r") as rf:
    ...
    plt.bar(opNames, opBinSizes, width=0.9, zorder=3, edgecolor="black", hatch="/")
...
#    plt.title('Executorch Operator Selective Build')
    plt.tight_layout()
    plt.savefig('operator_binary_sizes.png', format='png')
    plt.close()
...
with open("results.txt", "r") as rf:
    ...
    ax.bar([i+(w/2.0) for i in range(len(modelNames))], dtypeReduction, width=w, zorder=3, label="Dtype Selective Build", edgecolor='black', hatch="\\")
...
#    ax.set_title("Build Size Reduction with Dtype Selection")
    plt.margins(x=0.01)
    plt.tight_layout()
    plt.savefig('dtype_selective_build.png', format='png')
    plt.close()
...
#    ax.bar([i-0.00 for i in range(len(modelNames))], opReduction, width=w, zorder=3, label="Operator Selective Build", edgecolor='black', hatch="\\")
    ...
    plt.margins(x=0.01)
    plt.tight_layout()
    plt.savefig('dtype_selective_build_operator_baseline.png', format='png')
    plt.close()
...
#    ax.set_title("Low Data Dtype Diversity Across All Operators")
    plt.margins(x=0.01)
    plt.tight_layout()
    plt.savefig('dtype_diversity.png', format='png')
    plt.close()
...
with open("results.txt", "r") as rf:
    ...
    fig, ax = plt.subplots(figsize=(18,6))
    plt.grid(axis='y', zorder=0)
    ax.bar([i for i in range(len(modelNames))], numOps, width=0.9, zorder=3, edgecolor='black', hatch='/')
    ax.axvline(len(modelNames) - 1.5, color ='black', linestyle=':', lw = 2, alpha = 0.75)
    ax.set_xlabel('Model')
    ax.set_xticks([i for i in range(len(modelNames))], modelNames, rotation=65)
...
#    ax.set_title("High Operator Diversity Across Models")
    plt.margins(x=0.01)
    plt.tight_layout()
    plt.savefig('ops_per_model.png', format='png')
    plt.close()
